...
class FEMNISTDataset(Dataset):
    ...
    def __init__(self, client_names, data_dict):
        ...
        total_samples = sum(
            len(data_dict[name]["x"]) for name in self.client_names if name in data_dict
        )
        ...
        self.inputs = torch.empty((total_samples, 1, 28, 28), dtype=torch.float32)
        ...
        for client_name in self.client_names:
            if client_name in data_dict:
                n = len(data_dict[client_name]["x"])
                self.inputs[offset : offset + n] = (
                    torch.tensor(data_dict[client_name]["x"])
                    .view(-1, 1, 28, 28)
                    .float()
                )
                self.labels[offset : offset + n] = torch.tensor(
                    data_dict[client_name]["y"]
                ).long()
                offset += n
